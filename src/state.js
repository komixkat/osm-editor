...
export function undo() {
  ...
  EventBus.emit(EV.STATUS, `Undo (${AppState._historyPtr}/${AppState._history.length - 1})`);
}
...
export function redo() {
  ...
  EventBus.emit(EV.REDRAW_2D);
  EventBus.emit(EV.REBUILD_3D);
  EventBus.emit(EV.STATUS, `Redo (${AppState._historyPtr}/${AppState._history.length - 1})`);
}
