...
function _drawUserLayers(vb) {
  for (const layer of AppState.edits.userLayers) {
    ...
    if (layer.kind === 'tree') {
      ...
      ctx.fillStyle   = sel ? 'rgba(80,255,80,.6)' : 'rgba(30,160,50,.5)';
      ctx.strokeStyle = sel ? '#fff' : 'rgba(80,220,80,.5)';
    ...
    if (isLine) {
      ...
      ctx.strokeStyle = sel ? '#fff' : c.stroke;
...
function _drawBuildings(vb, sc) {
  ...
  for (const bld of AppState.osm.buildings) {
    ...
    ctx.fillStyle   = sel ? c.fill.replace(/[\d.]+\)$/, '0.9)') : c.fill;
    ctx.strokeStyle = sel ? '#fff' : c.stroke;
    ctx.lineWidth   = sel ? 1.5 : Math.max(0.5, sc * 0.008);
    ctx.fill();
    ctx.stroke();
    ...
    if (sc > 0.04) {
      ...
      ctx.fillStyle = sel ? '#fff' : c.stroke;
