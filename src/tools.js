...
import { draw, hitTest, unproj } from './renderer2d.js';

// renderer3d is lazily loaded — access via window._renderer3d set by main.js
const r3d = () => window._renderer3d || null;

// ─── TOOL DEFINITIONS ─────────────────────────────────────────────────────────
export const TOOLS = {
  select:   { label: '✦ Select',    key: 's', cursor: 'default',    group: 'select' },
  pan:      { label: '✋ Pan',       key: 'h', cursor: 'grab',       group: 'select' },
...
function _bind3DEvents() {
  _canvas3d.addEventListener('mousedown', e => {
    ...
    const osmId = r3d()?.hitTest3D(e.clientX, e.clientY);
    if (osmId) {
