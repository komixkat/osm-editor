...
import { AppState, EventBus, EV, updateBuildingEdit, updateUserLayer, getBuildingEdit, deleteUserLayer, undo, redo, pushHistory } from './state.js';
...
// renderer3d is loaded lazily — never import it directly.
// Communicate via EventBus: emit EV.REBUILD_3D to trigger rebuild.
// For camera mode, store in AppState.ui.cameraMode.
...
function _buildToolbar() {
  ...
  for (const [group, names] of Object.entries(groups)) {
    ...
    for (const name of names) {
      ...
      html += `<button class="tbtn" id="tbtn-${name}" data-tool="${name}" title="${def.label}${def.key ? ` (${def.key.toUpperCase()})` : ''}">${def.label}${key}</button>`;
    }
    html += '</div>';
  }

  // Undo/redo buttons — separate group on the right
  html += `<div class="tb-group tb-group-right">
    <button class="tbtn" id="tbtn-undo" title="Undo (Ctrl+Z)">↩ Undo</button>
    <button class="tbtn" id="tbtn-redo" title="Redo (Ctrl+Y)">↪ Redo</button>
  </div>`;
  ...
  document.getElementById('tbtn-undo')?.addEventListener('click', () => undo());
  document.getElementById('tbtn-redo')?.addEventListener('click', () => redo());
}
...
function _renderLayersTab() {
  ...
  // All buildings that have any edit (name, levels, type, deleted, etc.)
  const editedBuildings = buildings.filter(b => {
    const e = getBuildingEdit(b.id);
    return Object.keys(e).length > 0 && !e._deleted;
  });
  const deletedCount = buildings.filter(b => getBuildingEdit(b.id)._deleted).length;
  ...
  return `
    <div class="layer-summary">
      <div class="layer-row"><div class="ldot" style="background:#4d88ff"></div>Buildings<span>${buildings.length} total, ${editedBuildings.length} edited${deletedCount ? `, ${deletedCount} deleted` : ''}</span></div>
    ...
    <hr class="divider">
    <button class="btn-secondary" id="clear-user-layers">🗑 Clear all painted layers</button>
    <hr class="divider">
    <div class="field-label">Edited buildings <span class="field-sub">(click to select)</span></div>
    <div class="bld-list">
      ${editedBuildings.length === 0
        ? `<div class="field-hint">No buildings edited yet. Click a building with the Select tool to edit it.</div>`
        : editedBuildings.slice(0, 80).map(b => {
            const e   = getBuildingEdit(b.id);
            const nm  = e.name ?? b.tags.name ?? `#${b.id.slice(-6)}`;
            const lv  = e.levels ?? b.tags.levels ?? '';
            const typ = e.type ?? b.tags.building ?? '';
            const tag = lv ? `${lv}F` : typ ? typ : '';
            return `<div class="bld-item" data-id="${b.id}">${nm}${tag ? ` <span class="bld-tag">${tag}</span>` : ''}</div>`;
          }).join('')
      }
      ${editedBuildings.length > 80 ? `<div class="field-hint">…and ${editedBuildings.length - 80} more</div>` : ''}
    </div>
  `;
}
...
function _renderViewTab() {
  const camMode = AppState.ui.cameraMode || 'fly';
  return `
    ...
    <div class="btn-group">
      <button class="btn-toggle ${camMode==='fly'?'active':''}" id="cam-fly">✈ Fly (WASD)</button>
      <button class="btn-toggle ${camMode==='orbit'?'active':''}" id="cam-orbit">◎ Orbit</button>
    </div>
...
function _renderExportTab() {
  ...
  return `
    <div class="export-section">
      ...
      <div class="field-hint">Session JSON stores all your edits. Re-upload the .osm + session JSON to restore.</div>
    </div>
    <hr class="divider">
    <div class="export-section">
      ...
      ${hasOSM
        ? `<button class="btn-primary" id="exp-osm">↓ Export patched .osm</button>
           <div class="field-hint">Exports original OSM with building edits and painted layers baked in as OSM tags. Feed directly into Arnis.</div>`
    ...
    <hr class="divider">
    <div class="export-section">
      <div class="field-label" style="color:var(--danger,#e05555)">Danger zone</div>
      <button class="btn-danger" id="btn-clear-session">🗑 Clear all edits &amp; start over</button>
      <div class="field-hint">Clears all building edits and painted layers. The loaded .osm file stays. Cannot be undone.</div>
    </div>
  `;
}
...
function _bindTabEvents(tab) {
  ...
  if (tab === 'view') {
    ...
    document.getElementById('cam-fly')?.addEventListener('click', () => {
      AppState.ui.cameraMode = 'fly';
      EventBus.emit('camera:mode', 'fly');
    ...
    document.getElementById('tog-grid')?.addEventListener('change', e =>
      EventBus.emit('3d:toggle-grid', e.target.checked));
    document.getElementById('tog-blockgrid')?.addEventListener('change', e =>
      EventBus.emit('3d:toggle-blockgrid', e.target.checked));
    document.getElementById('tog-axes')?.addEventListener('change', e =>
      EventBus.emit('3d:toggle-axes', e.target.checked));
  }
  ...
  if (tab === 'export') {
    ...
    document.getElementById('btn-clear-session')?.addEventListener('click', () => {
      if (!confirm('Clear ALL building edits and painted layers?\n\nThis cannot be undone. The loaded .osm file will stay.')) return;
      AppState.edits.buildings  = new Map();
      AppState.edits.userLayers = [];
      AppState._history         = [];
      AppState._historyPtr      = -1;
      // Push fresh baseline so undo works after clear
      pushHistory();
      EventBus.emit(EV.REBUILD_3D);
      EventBus.emit(EV.REDRAW_2D);
      showTab('layers');
      setStatus('Session cleared — all edits removed');
    });
  }
}
