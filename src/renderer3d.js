...
export async function initRenderer3D(canvasEl) {
  ...
  // Renderer — use only safe r128 APIs, no outputColorSpace (r152+)
  renderer = new THREE.WebGLRenderer({ canvas: canvasEl, antialias: true, alpha: false });
  renderer.setPixelRatio(Math.min(devicePixelRatio, 2));
  renderer.shadowMap.enabled = true;
  // Safe numeric fallbacks for constants that may not exist in r128
  ...
  // Camera
  camera = new THREE.PerspectiveCamera(70, Math.max(canvasEl.clientWidth, 1) / Math.max(canvasEl.clientHeight, 1), 0.5, 8000);
